...
describe('Select', () => {
  ...
  it('check marked square with castling for white turn', () => {
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[5][0].marked).to.be.true;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[6][0].marked).to.be.true;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[3][0].marked).to.be.true;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[2][0].marked).to.be.undefined;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[3][1].marked).to.be.true;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[4][1].marked).to.be.true;
    expect(select(WHITE_CASTLING_POSITION, 4, 0).board[5][1].marked).to.be.undefined;

    expect(select(WHITE_CASTLING_POSITION, 4, 7).board[5][7].marked).to.be.undefined;
    expect(select(WHITE_CASTLING_POSITION, 4, 7).board[6][7].marked).to.be.undefined;
    expect(select(WHITE_CASTLING_POSITION, 4, 7).board[4][7].marked).to.be.undefined;
  });

  it('check marked square with castling for black turn', () => {
    expect(select(BLACK_CASTLING_POSITION, 4, 7).board[5][7].marked).to.be.undefined;
    expect(select(BLACK_CASTLING_POSITION, 4, 7).board[6][7].marked).to.be.undefined;
    expect(select(BLACK_CASTLING_POSITION, 4, 7).board[3][7].marked).to.be.true;
    expect(select(BLACK_CASTLING_POSITION, 4, 7).board[2][7].marked).to.be.true;
    expect(select(BLACK_CASTLING_POSITION, 4, 7).board[1][7].marked).to.be.undefined;
  });
});
